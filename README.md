...
以下依赖不确定是否要锁定版本，这里仅列一下测试时的版本，对于存量 app ，需特别注意，app 是否本来就已经安装过这些依赖来，是否存在版本冲突。

`yarn add react-native-reanimated@1.6.0 react-native-gesture-handler@1.5.3 react-native-archives@0.0.8`

可尝试升级为最新版，看看是否能正确运行

`yarn add react-native-reanimated react-native-gesture-handler react-native-archives`
